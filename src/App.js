...
//1.1 API間で使用する変数の定義
  ...
  const [massage, setMassage] = useState("");
    ...
    // const [contents, setComtents] = useState(""); ※Component内で定義
...
//1.4 Scroll関数 + 各種Trigger
  ...
  const handle1ButtonClick = () => {
    ...
    scrollToTop(selectQuestionsRef);
      // 問題文の表示（API連携後は削除）
    //if (selectedValue === "01:腕時計問題") {
      //setQuestionText("01:腕時計問題の問題文章ですああああ");
    //} else if (selectedValue === "02:電車業界問題") {
      //setQuestionText("02:電車業界問題の問題文章ですああああ");
    //} else {
      //setQuestionText("");
    //}
  };
  ...
  const handlePaymentComplete = () => {
    setContactForm(false); // フォームを非表示にする
  };
  ...
  const handleButtonClick_return = () => {
    ...
    //伊藤さん追加パート？←そう！
    setConfirmRecording(false); // 録音確認部分を初期状態に戻す　←これは効いてたけど同じコードあるなら消してOK
    setRecordedData(null); // 録音データをリセットして、以前の録音データをクリア　←これは効いてなかった（削除OK）
    scrollToTop(recordingRef); 
  };
    ...
    <div class="App">
      <header className="App-header">
        ...
        <div className="how-to-use">
          <p>「ケース面接Quest」はケース面接対策ができるアプリです<br/>
        ...
        <div className="consideration" ref={considerationRef}>
          {showConsideration ? (
            <div className="consideration-container">
              <p>【問題】</p>
              <p>{question}</p>
              <p>国内の腕時計の市場規模を簡易的に推定の上、市場の拡大施策を考えてください。</p>
        ...
        */}
        ...
        {showContactForm ? (
            <div>
              ...
              <ContactForm setName = {setName} setEmail = {setEmail} setMassage = {setMassage} APIsubmit = {APIsubmit} setAPIsubmit = {setAPIsubmit}  
